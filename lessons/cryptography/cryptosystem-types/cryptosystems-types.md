...
Alice encrypts a plaintext message with the shared private key, resulting in a ciphertext message. This encrypted ciphertext message is sent to Bob. Bob decrypts the ciphertext message with the shared private key, allowing him to recover the plaintext message sent by Alice. 
...
### RSA
Rivest-Shamir-Adleman (RSA) is a type of asymmetric encryption that uses a private and public key based on two large prime numbers. The algorithm for encryption involves multiplying two very large prime numbers, and is considered very hard to crack because recovering the two original prime numbers is a mathematical problem that is currently thought to be infeasible to solve efficiently. There are some known attacks against RSA, but many of these attacks occur when small prime numbers are used. For example, if small prime numbers are used, an attacker could create a dictionary by encrypting high probability plaintext messages using various keys, and compare the resulting ciphertext with a stolen ciphertext message. If the messages match, the attacker now knows the original prime numbers used to encrypt the message. 

### Diffie-Hellman
The Diffie-Hellman key exchange process involves the use of asymmetric encryption, and is used to establish a shared private key for use in future communications. When we previously talked about AES (symmetric cryptosystem), we noted that Alice and Bob would need a way to securely share a private key. The Diffie-Hellman key exchange process can accomplish this. The Diffie-Hellman algorithm involves a public modulus and base value, and two secret values chosen by Alice and Bob. Through an elegant mathematical algorithm, Alice and Bob can each use their own private secret values to calculate a public key that can be sent to each other unencrypted. They can then each use the other person's public key and their own secret value to calculate the same secret key. 

The strength of the Diffie-Hellman key exchange process depends on using a strong random number generator to choose the public modulus so that it cannot be easily predicted by an attacker. In addition, this process does not provide any method of authentication between Alice and Bob, so it is vulnerable to a man-in-the-middle attack where an attacker could intercept information between Alice and Bob and instead establish two separate Diffie-Hellman key exchanges with Alice and Bob. 
...
ECC is based on solving a challenging mathematical problem that is considered more difficult to solve and more computationally expensive than the mathematical problem that RSA and Diffie-Hellman are based on. With ECC, smaller keys will achieve the same level of security compared to RSA. 
...
In 2005, a cache-timing attack was performed on OpenSSL's AES encryption. It is known that side-channel attacks can occur, but this attack demonstrated obtaining the a complete AES private key. The time it takes to execute the AES algorithm on known plaintext messages can be used by attackers to to work backwards to determine the input. 

# Check YoSelf
## Q1 - Which cryptosystem is considered the most secure today? 
a. Diffie-Hellman <br>
b. AES <br> 
c. RSA <br>
d. ECC <---<br>

## Q2 - T/F: Symmetric cryptosystems involve public and private keys for the sender and receiver. 
a. True <br>
b. False <---<br>

## Q3 - Which of the following keys are used when Alice sends a message to Bob using asymmetric encryption? 
a. Alice's public key and Alice's private key <br>
b. Bob's public key and Bob's private key <---<br>
c. Alice's private key and Bob's private key <br>
d. Alice's public key and Bob's private key <br>

# Keep Going, Next Steps
Check out the following curated resources if you'd like to keep learning about this topic to dominate hard challenges.

1. AES Encryption - https://www.simplilearn.com/tutorials/cryptography-tutorial/aes-encryption
2. RSA Encryption - https://www.youtube.com/watch?v=ZPXVSJnDA_A
